...
import { acceptSuggestion } from '../parser.js';
import { readSidecar, writeSidecar } from '../sidecar.js';

export const acceptCommand = new Command('accept')
  .description('Accept a suggested edit')
  .argument('<file>', 'Markdown file')
  .requiredOption('--thread <id>', 'Thread ID of the suggestion to accept')
  ...
  .action((file: string, opts: { thread: string; author: string }) => {
    ...
    const newContent = acceptSuggestion(content, opts.thread);
    ...
    console.log(`Accepted suggestion ${opts.thread}`);
  });
